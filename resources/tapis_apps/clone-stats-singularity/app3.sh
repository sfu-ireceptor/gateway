...
#
    ...
    # Debugging output
    echo "IR-INFO: Input file = ${filename}"
...
# relative to that location.
{
    ...
    if [ ${#array_of_files[@]} != 1 ]
    then
        echo "IR_ERROR: Clone stats analysis only works with a single cell file."
        return
    fi
    filename=${array_of_files[0]}

    # Keep track of our base file string, use repertoire_id
    file_string=${repertoire_id}

	# Check to see if we are processing a specific repertoire_id
    # If so generate a reasonable title.
