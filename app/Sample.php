...
class Sample
{
    ...
    public static function cache_epitope_counts($username, $rest_service_id = null)
    {
        ...
        foreach ($response_list as $i => $response) {
            ...
            $num_repertoires = count($repertoire_list);
            $count = 0;
            foreach ($repertoire_list as $repertoire_id) {
                ...
                Log::debug('Processing species for repertoire ' . $count . ' of ' . $num_repertoires);
                $count = $count + 1;
                $timeout_scale = 4;
                $species_array = RestService::object_list('sequence', [$rest_service_id => [$repertoire_id]], [], 'ir_species_ref', $timeout_scale);
                ...
                foreach ($species_array[$rest_service_id] as $species_list) {
                    ...
                    if ($species_list != null) {
                        ...
                        // If it isn't a list, it is an error
                        if (is_array($species_list)) {
                            ...
                            Log::error('Sample::cache_epitopes - invalid count repsonse.');
                        }
                    }
                }
            }
