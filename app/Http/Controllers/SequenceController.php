        ...
        foreach ($appTemplates as $app_tag => $app_info) {
            ...
            if (array_key_exists('time_secs_per_million', $app_info)) {
                ...
                if ($required_time_secs > $job_runtime_secs) {
                    ...
                    $app_ui_info['runnable'] = false;
                    $error_string = 'It is estimated that "' . $app_ui_info['name'] . '" will require ' . secondsToTime($required_time_secs) . ' minutes to process ' . human_number($num_objects) . ' rearrangements. Current maximum job run time is ' . secondsToTime($tapis->maxRunTimeMinutes()*60) . ' minutes. Please limit the amount of data used for this analysis.';
